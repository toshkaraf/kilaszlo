...
class DeepSeekService {
  ...
  static const String _modelId = 'deepseek-chat';

  String get _apiKey => (dotenv.env['DEEPSEEK_API_KEY'] ?? '').trim();
  ...
  Future<http.Response> _postChat(String prompt, {required int maxTokens}) {
    final body = {
      'model': _modelId,
      'messages': [
  ...
  Future<String> _extractTextFromResponse(http.Response response) async {
    if (response.statusCode != 200) {
      ...
      debugPrint('[DeepSeek API] HTTP ${response.statusCode}');
      debugPrint('[DeepSeek API] $errorBody');
  ...
  Future<String> generateAIResponse(
    ...
    try {
      ...
      final response =
          await _postChat(promptText.toString(), maxTokens: 1024);
  ...
  Future<List<String>> generateInitialFactSuggestions(String topic) async {
    try {
      ...
      final response = await _postChat(prompt, maxTokens: 400);
